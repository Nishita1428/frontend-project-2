...
restart_quiz.onclick = ()=>{
    ...
    clearInterval(counterLine); //clear counterLine//remove comment
    startTimer(timeValue); //calling startTimer function
    startTimerLine(widthValue); //calling startTimerLine function//remove comm
    timeText.textContent = "Time Left"; //change the text of timeText to Time Left//remove comm
...
function optionSelected(answer){
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        userScore += 5; //upgrading score value with 5
...
function nextQuestion() {
    if (que_count < questions.length - 1) { // if there are more questions
        ...
        startTimerLine(widthValue); // start the timer line for the next question
        timeText.textContent = "Time Left"; // change the timeText to "Time Left"
    } else {
        clearInterval(counter); // clear counter
        clearInterval(counterLine); // clear counterLine
    ...
    if (userScore > 15){ // if user scored more than 15
        ...
        let scoreTag = '<span> Hi <P>, '+ name +'</p> You got <p>'+ userScore +'</p> out of <p>'+ questions.length*5 +'</p></span>';
        ...
        // let scoreTag = '<span> , You got only <p>'+ userScore +'</p> out of <p>'+ questions.length*5 +'</p></span>';
        // scoreText.innerHTML = scoreTag;
...
function startTimer(time){
    ...
    function timer(){
        ...
        // if (time < 0) { // if timer is less than 0
            // clearInterval(counter); // clear counter
            // timeText.textContent = "Time End"; // change the timeText to indicate time is up
